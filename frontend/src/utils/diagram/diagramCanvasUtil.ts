...
export const getCanvasTypes = (
    canvas_type?: string //
) => {
    if (!canvas_type) return [];

    if (canvas_type === CanvasType.threat_scenario.toString()) {
        return [CanvasType.threat_scenario.toString()];
    }
    return [
        CanvasType.architecture.toString(),
        CanvasType.data_flow.toString(),
        CanvasType.summary.toString(),
    ];
};
...
export const getOptionsByKey = (
    ...
    return canvases
        ?.filter((c: DiagramCanvas) => c.ref.threat_scenario_ref?.status === key)
        ?.map((c) => {
            const ranking = c?.ref?.threat_scenario_ref?.ranking;
            return {
                label: c?.canvas_name,
                value: c?.canvas_id,
                startIcon: typeof ranking === "number" ? `#${ranking}` : undefined,
            };
        });
};

export const getCanvasViewOptions = (
    canvas: DiagramCanvas[], //
    canvas_types: string[]
) => {
    const groupedOptions = [];

    const architecture_options = canvas
        ?.filter((c) => c?.canvas_type === CanvasType.architecture.toString())
        ?.map((c) => {
            return {
                label: c?.canvas_name, //
                value: c?.canvas_id,
    ...
    if (!!canvas_types?.includes(CanvasType.architecture.toString())) {
        groupedOptions.push({
            label: "Architecture", //
    ...
    const data_flow_options = canvas
        ?.filter((c) => c?.canvas_type === CanvasType.data_flow.toString())
        ?.map((c) => {
            return { label: c?.canvas_name, value: c?.canvas_id };
        });
    if (!!canvas_types?.includes(CanvasType.data_flow.toString())) {
        groupedOptions.push({
            label: "Data Flow",
    ...
    const llm_options = canvas
        ?.filter((c) => c?.canvas_type === CanvasType.llm.toString())
        ?.map((c) => {
            return { label: c?.canvas_name, value: c?.canvas_id };
        });
    if (!!canvas_types?.includes(CanvasType.llm.toString())) {
        groupedOptions.push({
            label: "LLM",
            options: llm_options,
        });
    ...
    if (!!canvas_types?.includes(CanvasType.summary.toString())) {
        groupedOptions.push({
            label: CanvasType.summary.toString(),
            options: [{ label: "Summary", value: CanvasType.summary.toString() }],
        });
    }
