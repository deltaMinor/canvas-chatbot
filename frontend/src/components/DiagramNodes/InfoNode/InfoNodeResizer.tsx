...
interface NodeResizerProps extends NodeProps<DiagramNode> {
    min_width: number;
    min_height: number;
}

const NodeResizerComponent = (props: NodeResizerProps) => {
    ...
    const shouldResize: ShouldResize = React.useCallback(
        (_event: ResizeDragEvent, _params: ResizeParams) => {
            return false;
        },
        []
    );
    ...
    const onResizeEnd: OnResizeEnd = React.useCallback(
        async (_event: ResizeDragEvent, params: ResizeParams) => {
            ...
            try {
                ...
                if (nodeIndex < 0) throw new Error("Node cannot be found.");
...
export default React.memo(NodeResizerComponent);
