...
class ProjectADApplicationService(ProjectADService):
    ...
    def insert_project_ad_image_files(
        ...
        for uploaded_file in image_files:
            ...
            if not FileValidator.is_image_content_type(
                content_type
            ) and content_type != "application/pdf":
                raise BadRequest(
                    f"File {uploaded_file.name} must be an image or PDF."
                )
            raw = FileValidator.read_uploaded_file(uploaded_file)
            if not FileValidator.is_non_empty(raw):
                raise BadRequest(f"File {uploaded_file.name} is empty.")
            if content_type == "application/pdf" and not FileValidator.is_valid_pdf(raw):
                raise BadRequest(f"File {uploaded_file.name} is not a valid PDF.")
            encoded_file = base64.b64encode(raw).decode("utf-8")
