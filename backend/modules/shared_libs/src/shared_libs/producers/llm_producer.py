...
class LLMProducer:
    ...
    @raise_exception(
        "Failed to run async in llm producer.",
        exception_logger=logger,
    )
    def run_async(
        self,
        task_type: str = "run_llm",
        **kwargs,
    ):
        logger.info("[ SHARED-PRODUCER ] Starting async llm task ...")
        task_body = {**kwargs}
    ...
    @raise_exception(
        "Failed to run sync in llm producer.",
        exception_logger=logger,
    )
    def run_sync(
        self,
        ...
        **kwargs,
        ...
        logger.info("[ SHARED-PRODUCER ] Starting sync llm task ...")
        task_body = {**kwargs}
