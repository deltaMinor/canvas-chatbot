...
default_code_message_mapping = {
    404: "Master CQ not found.",
    409: "Master CQ conflict.",
}

logger = logging.getLogger(__name__)


class MasterCQService(
    DomainRepositoryService,
    DomainRequestDataPrefilterService,
    DomainAuditLogService,
    DomainAuthorizationService,
):
    """Service for managing master CQ entities.

    This service provides methods to interact with master CQ entities,
    ...
    Attributes:
        SCHEMA (dict): The schema definition for master CQ entities.
        repository (Repository | RemoteRepository): The repository instance for data access.
        data_store (GenericDataStore, optional): An optional data store for master CQ retrieval.
    """

    SCHEMA = schema_dict["master_cq"]

    def __init__(
        self,
        repository: "Repository | RemoteRepository",
    ):
        """Initializes the MasterCQService.
