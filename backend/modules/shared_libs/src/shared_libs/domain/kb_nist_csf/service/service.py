...
default_code_message_mapping = {
    404: "Kb NIST CSF not found.",
    409: "Kb NIST CSF conflict.",
}

logger = logging.getLogger(AppLoggerName.domain_kb_nist_csf.value)


class KbNistCSFService(
    DomainRepositoryService,
    DomainRequestDataPrefilterService,
    DomainAuditLogService,
    DomainAuthorizationService,
):
    """Service for managing Kb NIST CSF entities.

    This service provides methods to interact with Kb NIST CSF entities,
    ...
    Attributes:
        SCHEMA (dict): The schema definition for Kb NIST CSF entities.
        repository (Repository | RemoteRepository): The repository instance for data access.
        data_store (GenericDataStore, optional): An optional data store for Kb NIST CSF retrieval.
    """

    SCHEMA = schema_dict["kb_nist_csf"]
