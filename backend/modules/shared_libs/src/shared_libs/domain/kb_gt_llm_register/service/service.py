...
default_code_message_mapping = {
    404: "Kb Govtech LLM Register not found.",
    409: "Kb Govtech LLM Register conflict.",
}

logger = logging.getLogger(__name__)


class KbGTLLMRegisterService(
    DomainRepositoryService,
    DomainRequestDataPrefilterService,
    DomainAuditLogService,
    DomainAuthorizationService,
):
    """Service for managing Kb Govtech LLM Register.

    This service provides methods to interact with Kb Govtech LLM Register,
    ...
    Attributes:
        SCHEMA (dict): The schema definition for Kb Govtech LLM Register.
        repository (Repository | RemoteRepository): The repository instance for data access.
        data_store (GenericDataStore, optional): An optional data store for Kb Govtech LLM Register retrieval.
    """

    SCHEMA = schema_dict["kb_gt_llm_register"]

    def __init__(
        self,
        repository: "Repository | RemoteRepository",
    ):
        """Initializes the KbGTLLMRegisterService.
