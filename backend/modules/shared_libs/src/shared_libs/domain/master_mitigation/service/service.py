...
default_code_message_mapping = {
    404: "Master mitigation not found.",
    409: "Master mitigation conflict.",
}

logger = logging.getLogger(__name__)


class MasterMitigationService(
    DomainRepositoryService,
    DomainRequestDataPrefilterService,
    DomainAuditLogService,
    DomainAuthorizationService,
):
    """Service for managing master mitigation entities.

    This service provides methods to interact with master mitigation entities,
    ...
    Attributes:
        SCHEMA (dict): The schema definition for master mitigation entities.
        repository (Repository | RemoteRepository): The repository instance for data access.
        data_store (GenericDataStore, optional): An optional data store for master mitigation retrieval.
    """

    SCHEMA = schema_dict["master_mitigation"]

    def __init__(
        self,
        repository: "Repository | RemoteRepository",
    ):
        """Initializes the MasterMitigationService.
