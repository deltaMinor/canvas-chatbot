...
default_code_message_mapping = {
    404: "Project assessment config file not found.",
    409: "Project assessment config file conflict.",
}

logger = logging.getLogger(__name__)


class ProjectAssessmentConfigFileService(
    DomainRepositoryService,
    DomainRequestDataPrefilterService,
    DomainAuditLogService,
    DomainProjectAuthorizationService,
    ...
    @raise_exception(
        "Failed to get multiple documents from domain.",
        exception_logger=logger,
    )
    ...
    @raise_exception(
        "Failed to delete multiple documents from domain.",
        exception_logger=logger,
    )
