...
default_code_message_mapping = {
    404: "Master CQ template not found.",
    409: "Master CQ template conflict.",
}

logger = logging.getLogger(__name__)


class MasterCQTemplateService(
    DomainRepositoryService,
    DomainRequestDataPrefilterService,
    DomainAuditLogService,
    DomainAuthorizationService,
):
    """Service for managing master CQ template entities.

    This service provides methods to interact with master CQ template entities,
    ...
    Attributes:
        SCHEMA (dict): The schema definition for master CQ template entities.
        repository (Repository | RemoteRepository): The repository instance for data access.
        data_store (GenericDataStore, optional): An optional data store for master CQ template retrieval.
    """

    SCHEMA = schema_dict["master_cq_template"]

    def __init__(
        self,
        repository: "Repository | RemoteRepository",
    ):
        """Initializes the MasterCQTemplateService.
